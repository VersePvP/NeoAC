...
import dev.tawny.Voit.manager.PunishmentManager;
...
public abstract class Check {
    ...
    public final void fail(final Object info) {
        ...
        if (!data.isExempt() && enabled) {
            ...
            if(Voit.INSTANCE.getTickManager().getA() > 102) {
    ...
    public final void kick(final String reason) {
        if (!data.isExempt()) {
            fail();
            Bukkit.getScheduler().runTask(Voit.INSTANCE.getPlugin(), () -> data.getPlayer().kickPlayer(reason));
        }
    }
    ...
    public final int ticks() { return Voit.INSTANCE.getTickManager().getTicks(); }
    ...
    public final void debug(final Object object) {
        if (debug) {
            data.getPlayer().sendMessage(ChatColor.GRAY + "[" + ChatColor.RED + "Voit-Debug" + ChatColor.GRAY + "] " + ChatColor.GRAY + object);
        }
    }

    public final void broadcast(final Object object) {
        Bukkit.broadcastMessage(ChatColor.GRAY + "[" + ChatColor.RED + "Voit-Debug" + ChatColor.GRAY + "] " + ChatColor.GRAY + object);
    }
